...
class DatabaseSeeder extends Seeder {
    ...
    private function seedUsers(Collection $roles, int $count): Collection {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $cityManager = $this->seedUser($this->faker->name, $this->faker->email, $userPassword,
                                        rand(10000000000000, 99999999999999), $userAvatarUrl);
            $cityManager->roles()->attach($cityManagerRole);
            $users->add($cityManager);
            
    
            $gymManager = $this->seedUser($this->faker->name, $this->faker->email, $userPassword,
                                        rand(10000000000000, 99999999999999), $userAvatarUrl);
            $gymManager->roles()->attach($gymManagerRole);
            $users->add($gymManager);
    
    
            $coach = $this->seedUser($this->faker->name, $this->faker->email, $userPassword,
                                        rand(10000000000000, 99999999999999), $userAvatarUrl);
            $coach->roles()->attach($coachRole);
            $users->add($coach);
    
    
            $member = $this->seedUser($this->faker->name, $this->faker->email, $userPassword,
                                        rand(10000000000000, 99999999999999), $userAvatarUrl);
